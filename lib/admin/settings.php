...
function mai_render_admin_menu_page() {
	echo '<div class="wrap">';
		...
		?>
		<div class="mai-plugin-pack-cta">
			<div class="mai-plugin-pack-cta-inner">
		...
		$has_premium = wp_list_pluck( $premium_plugins, 'hide' );
		$has_premium = in_array( false, $has_premium, true );
		$has_free    = wp_list_pluck( $free_plugins, 'hide' );
		$has_free    = in_array( false, $has_free, true );

		if ( $has_premium ) {
			printf( '<h2 style="margin-top:40px;font-size:2em;">%s</h2>', __( 'Premium Add-on Plugins', 'mai-engine' ) );
		...
		if ( $has_free ) {
			printf( '<h2 style="margin-top:40px;font-size:2em;">%s</h2>', __( 'Free Add-on Plugins', 'mai-engine' ) );
